...
private:
	...
	void percolateDown(unsigned int hole)
	{
		...
		int leftChild, rightChild, index = hole;
		leftChild = hole * 2;
		rightChild = (hole * 2) + 1;
		T temp;
		
		if (leftChild < this->size() && this->_items[leftChild] < this->_items[index]) //see if left is lower than current
		{
			index = leftChild; //percolate on down to the left
		}
		if (rightChild < this->size() && this->_items[rightChild] < this->_items[index]) //see if right side is lower
		{
			index = rightChild; //percolate on down to the right
		}
		if (index != hole) //if we still arent where we need to be
		{
			temp = _items[index]; //we swap elements
			_items[index] = _items[hole];
			_items[hole] = temp;
			percolateDown(index); //and instead of loop we call recursively 
		}
	}
	...
	void percolateUp(T item)
	{
		...
		this->_items.push_back(item); // add new item to end of heap;

		int itemIndex = size(); //get index of that last value
		int ancestorIndex; //used to keep track of parent
		T temp;

		while (_items[itemIndex / 2] > _items[itemIndex]) //while parent is bigger than child
		{
			ancestorIndex = itemIndex / 2; //find parent
			temp = _items[itemIndex];
			_items[itemIndex] = _items[ancestorIndex];
			_items[ancestorIndex] = temp; //these lines do the swapping

			itemIndex = ancestorIndex; //progress up the heap
		}
	}
